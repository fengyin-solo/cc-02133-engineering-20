<template>
  <div class="case-page">
    ...
    <section class="filter-section">
      <div class="container">
        <div class="filter-bar">
          ...
          <el-button class="baseline-entry" :icon="SetUp" plain @click="baselineDialogVisible = true">
            构建基线
            <el-badge
              v-if="currentIssueCount > 0"
              :value="currentIssueCount"
              :type="currentErrorCount > 0 ? 'danger' : 'warning'"
    ...
    <!-- 成功案例构建基线管理 -->
    <CaseBaselineDialog v-model="baselineDialogVisible" />
  </div>
</template>
...
import CaseBaselineDialog from '@/components/CaseBaselineDialog.vue'
...
// 成功案例来自构建基线（内置基线 / 导入基线），首次进入与前进后退保持一致
const { cases, currentReport } = useCaseStore()

const filteredCases = computed(() => {
  if (activeTag.value === 'all') {
    return cases.value
  }
  return cases.value.filter(c => c.tag === activeTag.value)
})

// 基线入口徽标：当前基线存在缺失/重复/漂移时给出提示
const currentIssueCount = computed(() => currentReport.value.issues.length)
const currentErrorCount = computed(() => currentReport.value.errors.length)
